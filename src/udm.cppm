...
export namespace udm {
	...
	{
		auto it = std::find_if(children.begin(), children.end(), [&child](const std::pair<std::string, PProperty> &pair) { return pair.second->type == Type::Element && pair.second->value == &child; });
		if(it == children.end())
			return;
		children.erase(it);
	}
